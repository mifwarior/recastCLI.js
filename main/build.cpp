...
)
{
	...
	if(file.is_open()) {
		...
		std::string str = buildObj(content,cellSize, cellHeight, agentHeight, agentRadius, agentMaxClimp, agentMaxSlope, regionMinSize, regionMergeSize,edgeMaxLen, edgeMaxError, vertsPerPoly, detailSampleDist, detailSampleMaxError);
...
//                              // --------------------------- Initialize
  ...
  if (!m_geom->loadContent(m_ctx, arrayBuffer))
  {
    m_ctx->log(RC_LOG_ERROR, "Cannot load obj");
    return "";
  }
  ...
  //
  // Step 7. Create detail mesh which allows to access approximate height on each polygon.
  // Step 7: Create a detail mesh and generate height details
  // In this final stage, the convex polygon mesh will be triangulated by the Delaunay triangulation algorithm, which can increase the height of detail.
  //
  ...
  // Now the Navmesh data is available, you can access the data through m_pmesh
  // duDebugDrawPolyMesh and dtCreateNavMeshData have access use cases
  ...
  do
  {
    ...
    for (int i = 0; i < m_pmesh->npolys; i++)
    {
      ...
      for (int v = 0; v < m_pmesh->nvp; v++)
      {
        if (poly[v] == RC_MESH_NULL_IDX)
        {
          // if the current vertex is empty
          break;
        }
        else
        {
